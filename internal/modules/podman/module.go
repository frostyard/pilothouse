...
func (*Module) RequiredCapabilities() []capability.ID {
	return []capability.ID{capability.Podman}
}

func (m *Module) Mount(mux *http.ServeMux, host platform.Host) {
	mux.HandleFunc("GET /podman", platform.Gate(host, []capability.ID{capability.Podman}, func(w http.ResponseWriter, r *http.Request) {
	...
	mux.HandleFunc("GET /podman/containers/{id}/logs", platform.Gate(host, []capability.ID{capability.Podman}, func(w http.ResponseWriter, r *http.Request) {
		id := r.PathValue("id")
	...
	mux.HandleFunc("POST /podman/containers/{id}/{action}", platform.Gate(host, []capability.ID{capability.Podman}, func(w http.ResponseWriter, r *http.Request) {
		...
		if (action == "stop" || action == "remove") && !host.ConfirmAction(w, r, confirmTitles[action], "podman/container/"+id) {
			return
		}
	...
	mux.HandleFunc("POST /podman/images/{id}/{action}", platform.Gate(host, []capability.ID{capability.Podman}, func(w http.ResponseWriter, r *http.Request) {
		if !host.ValidateAction(w, r) {
			return
		}
