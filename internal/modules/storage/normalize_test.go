...
func TestNormalizeRejectsMissingRelationEndpoint(t *testing.T) {
	_, err := normalize(time.Unix(1, 0), []collectedResult{{result: AdapterResult{Resources: []Resource{{ID: "a"}}, Relations: []Relation{{From: "a", To: "missing"}}}}})
...
func TestNormalizeRejectsDepthOverLimit(t *testing.T) {
	...
	_, err := normalize(time.Unix(1, 0), []collectedResult{{result: AdapterResult{Resources: resources, Relations: relations}}})
