...
func TestUCoreContainerfileUsesOfflineReleasedRPMAndBootcLint(t *testing.T) {
	...
	require.Contains(t, instructions, "FROM ${UCORE_IMAGE}")
	require.Contains(t, instructions, "COPY ${PILOTHOUSE_RPM} /tmp/pilothouse-image-test.rpm")
	...
	require.Contains(t, instructions, "RUN bootc container lint")
	require.Contains(t, instructions, `LABEL io.frostyard.pilothouse.image-test="true"`)
	for _, instruction := range instructions {
		require.NotContains(t, instruction, "rpm-ostree install")
		require.NotContains(t, instruction, "curl ")
		require.NotContains(t, instruction, "wget ")
	}
}
...
func imageWriteRPMFixture(t *testing.T, workspace string) {
	...
	manifest := `{"schema":1,"kind":"pilothouse-release-rpm-fixture","artifact":"` +
...
case "${1-}" in
  ...
  build)
    ...
    [ "${10}" = "--build-arg" ] &&
      [ "${11}" = "PILOTHOUSE_RPM=frostyard-pilothouse-0.6.0-1.x86_64.rpm" ] || exit 65
    [ "${12}" = "--build-arg" ] || exit 65
    slot_arg="${13}"
    [ "${14}" = "$PWD/fixture-release-rpm" ] || exit 65
