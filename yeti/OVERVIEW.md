...
- **Capability probing at startup.** `pilothoused` probes optional host
  ...
  (`broker.QueryCapabilities`), returning `{"capabilities": [...]}` (present
  IDs only, sorted).
- **Capability-gated, non-fatal registration.** Every `registerX` function in
  `cmd/pilothoused/main.go` (services, logs, backups, storage remote-mount
  actions, maintenance, sysext, Podman, Docker, Incus) takes the probed
  `capability.Set` and only registers its broker IDs when the relevant
  capability is present, instead of failing `run()` when an optional
  dependency is absent. `docs/capabilities.md` is the binding table mapping
  every broker ID to its required capability (or capabilities); most modules
  share one requirement per function, but sysext is guarded per-action
  (`ActionSysextDisable`/`ActionSysextEnable` need `updex AND sysext`,
  `ActionSysextRefresh` needs `sysext` alone, `ActionSysextUpdate` needs
  `updex` alone) and services/logs split `systemd`-only reads from the
  `systemd AND journald` journal queries. `QueryStorageState` (plain
  inventory) is the one query registered unconditionally regardless of
  systemd, because its manager never dials D-Bus — only the remote-mount
  *actions* need systemd.
- **Systemd-backed managers accept a pre-opened, non-fatal connection.**
  `backups.NewSystemManager`, `services.NewSystemManager`, and
  `logs.NewSystemManager` no longer dial system D-Bus themselves; each
  accepts a pre-opened client from its caller. `connectSystemd` in
  `cmd/pilothoused/main.go` opens that one shared connection at most once,
  only when the probed `Systemd` capability is present, using a bounded
  context (mirroring the probe's own timeout) rather than
  `context.Background()` — reusing an unbounded context for this second dial
  would reintroduce the unbounded-startup-hang risk the probe exists to rule
  out. A connect failure degrades to a nil client and a warning log, never a
  fatal `run()` error; `buildSystemdManagers` then constructs the
  remote-mount controller and backups/services/logs managers only when that
  client is non-nil. Maintenance and sysext manager construction have no
  systemd dependency of their own (they're exec/`CommandRunner`-based) and
  are always constructed; only their broker *registration* is
  capability-guarded. New modules that wrap an optional host resource should
  follow the same shape: accept a pre-opened/pre-resolved dependency from the
  caller, gate opening it on the relevant probed capability, and never let
  its absence fail `run()`.
- **Maintenance degrades its extension-read subpath, not just registration.**
  `maintenance.SystemManager.State`'s `extensionState` helper (feeding
  `Updates` and merged-status-derived reboot reasons) takes
  `updexAvailable`/`sysextAvailable` booleans (from the probed capabilities)
  and skips only the parts that need the missing tool instead of erroring:
  with `updex` present but `sysext` absent, `Updates` still populates but
  feature/reboot-reason enumeration is skipped; with `updex` absent, neither
  populates. `State` never returns an error for missing updex/sysext in any
  combination. See `docs/capabilities.md`'s extension-read note for the full
  table.
