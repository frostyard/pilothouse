...
If native Go, PAM, or systemd build dependencies are unavailable, use the matching containerized targets: `make docker-build`, `make docker-test`, `make docker-fmt`, and `make docker-lint`. Use `make docker-generate` after templ changes. These targets build and reuse the repository's development image; do not assemble ad hoc build containers when they are available. The editor devcontainer installs Go-based tools at explicit release versions through Go's checksum-verified module download path; do not replace those installs with floating versions or remote install scripts.
